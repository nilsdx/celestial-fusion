---
name: Hell Needle
icon: ra
image: ""
type: Needle
requirement: 135 ATA
grind: 60
targets: 5
special: Hell
stats:
  - label: ATP
    value: "400"
  - label: ATA
    value: "40"
classes: "000011110000"
description: A custom shotgun that shoots countless needles. Its special attack causes instant death.
---
# Hell Needle

Hell Needle is a rare Needle weapon. Similarly to Arrest Needle, it has a high tier special in the form of Hell.

When combined with Vector Scope, it becomes the best Hell weapon for Rangers due to its speed and its range being better than a Shot weapon.

## Set Effect

| Item | Effect |
| ----------- | ----------- |
| [Vector Scope](/units/vector-scope) | Range +105.0, ATA+20 |

## Availability

### Item Combination

| Material 1 | Grind | Material 2 | Class | Stats | Result |
| ----------- | ----------- | ----------- | ----------- | ----------- | ----------- |
| [Spread Needle](https://wiki.pioneer2.net/w/Spread_Needle) | Any | [Dark Matter](/tools/dark-matter) | Any Ranger | Any | Hell Needle |
